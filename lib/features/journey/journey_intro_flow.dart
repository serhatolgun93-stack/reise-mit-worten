...
class _JourneyIntroFlowState extends State<JourneyIntroFlow> {
    ...
    return Scaffold(
      ...
      body: Stack(
        ...
        children: [
          ...
          const DecoratedBox(decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0x33000000), Color(0x66000000), Color(0xEE0A090C)], stops: [0, .5, 1]))),
          SafeArea(
            child: Padding(
              ...
              child: Column(children: [
                Row(children: [
                  ...
                  const Spacer(),
                  Text('${_step + 1} / 6', style: const TextStyle(color: Color(0xFFE4DEE6), fontWeight: FontWeight.w800)),
                ]),
                const Spacer(),
                Flexible(child: SingleChildScrollView(child: AnimatedSwitcher(duration: const Duration(milliseconds: 300), child: _buildStep()))),
                const SizedBox(height: 16),
                SizedBox(width: double.infinity, height: 58, child: FilledButton(
                  onPressed: _next,
                  style: FilledButton.styleFrom(backgroundColor: const Color(0xFFFF2E9A), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18))),
                  child: Text(_step == 5 ? 'Meine Reise beginnen' : 'Weiter', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900)),
                )),
              ]),
            ),
          ),
    ...
    switch (_step) {
      case 0:
        return _card('welcome', Column(children: [
          Container(width: 92, height: 92, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: const Color(0xFFFF75BD), width: 3), image: DecorationImage(image: AssetImage(widget.backgroundAsset), fit: BoxFit.cover, alignment: const Alignment(.25, 0)))),
          const SizedBox(height: 16),
      ...
      case 1:
        return _card('name', Column(children: [
          ...
          TextField(controller: _nameController, autofocus: true, textCapitalization: TextCapitalization.words, textInputAction: TextInputAction.done, style: const TextStyle(color: Color(0xFF171217), fontSize: 18), decoration: InputDecoration(hintText: 'Vorname', filled: true, fillColor: const Color(0xFFF7F2F5), contentPadding: const EdgeInsets.symmetric(horizontal: 18, vertical: 16), border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide.none)), onSubmitted: (_) => _next()),
        ]));
      case 2:
        return _card('hello', Column(children: [
          ...
          const SizedBox(height: 14), Text(widget.greeting, style: _accent),
        ]));
      case 3:
        return _card('goals', Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ..._goalOptions.map((goal) => CheckboxListTile(
            value: _goals.contains(goal), dense: true, contentPadding: EdgeInsets.zero,
            activeColor: const Color(0xFFFF2E9A), checkColor: Colors.white,
      ...
      case 4:
        return _card('journal', Column(children: [
          ...
          Container(width: double.infinity, padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: const Color(0x22FF75BD), borderRadius: BorderRadius.circular(18), border: Border.all(color: const Color(0x55FF75BD))), child: Text('Erster Eintrag\n\nHeute beginnt $_names Reise mit einem kleinen Schritt – und einem neuen Wort. ${widget.greeting}!', textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, height: 1.5, fontSize: 15))),
        ]));
      default:
        return _card('ready', Column(children: [
          const Icon(Icons.flight_takeoff_rounded, color: Color(0xFFFF2E9A), size: 56),
          const SizedBox(height: 16),
          ...
          if (_goals.isNotEmpty) ...[
            const SizedBox(height: 20),
            Text('${_goals.length} persönliche ${_goals.length == 1 ? 'Reiseidee' : 'Reiseziele'} gespeichert', style: const TextStyle(color: Color(0xFFD5CFD8), fontWeight: FontWeight.w700)),
          ],
        ]));
    }
  }

  String get _names => _name.endsWith('s') ? _name : '${_name}s';

  Widget _card(String keyName, Widget child) => Container(key: ValueKey(keyName), width: double.infinity, padding: const EdgeInsets.fromLTRB(22, 24, 22, 24), decoration: BoxDecoration(color: const Color(0xE6171218), borderRadius: BorderRadius.circular(28), border: Border.all(color: const Color(0x55FFFFFF)), boxShadow: const [BoxShadow(color: Color(0x55000000), blurRadius: 28, offset: Offset(0, 12))]), child: child);

  void _back() {
    FocusScope.of(context).unfocus();
    if (_step == 0) Navigator.of(context).pop(); else setState(() => _step--);
  }
  ...
  void _next() {
    ...
    if (_step < 5) { setState(() => _step++); return; }
    Navigator.of(context).pushReplacement(MaterialPageRoute<void>(builder: (_) => JourneyStartScreen(language: widget.language, flag: widget.flag, greeting: widget.greeting)));
  }
