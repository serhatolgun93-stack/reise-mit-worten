...
class _JourneyIntroFlowState extends State<JourneyIntroFlow> {
  ...
  String get _guideAsset {
    ...
    if (_step == 4) return _isLeon ? 'assets/characters/leon_invite.png' : 'assets/characters/maria_welcome.png';
  ...
  Widget _buildStep(Size s,bool l) {
    if (_step == 1) return l ? _stepOneLandscape(s) : _stepOnePortrait(s);
    if (_step == 2) return l ? _stepTwoLandscape(s) : _stepTwoPortrait(s);
    if (_step == 3) return l ? _stepThreeLandscape(s) : _stepThreePortrait(s);
    return l ? _stepFourLandscape(s) : _stepFourPortrait(s);
  }

  Widget _guideLayer(Size s,bool l) {
    final right=_step==2 || _step==4;
    return Positioned(
      left:right?null:(l?-12:-28),
      right:right?(l?-8:-32):null,
      bottom:l?-10:((_step==3||_step==4)?105:120),
      child:IgnorePointer(child:SizedBox(
        width:l ? s.width * ((_step==3||_step==4) ? 0.27 : 0.32) : s.width * ((_step==3||_step==4) ? 0.43 : 0.62),
        height:l ? s.height * 0.88 : s.height * ((_step==3||_step==4) ? 0.43 : 0.58),
  ...
  Widget _stepThreePortrait(Size s)=>Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[_header('3 / 8',()=>setState(()=>_step=2)),const Spacer(),Align(alignment:Alignment.centerRight,child:_levelCard(s.width*.72,false)),const SizedBox(height:12),_continueLevelButton(height:58)]);
  Widget _stepThreeLandscape(Size s)=>Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[_header('3 / 8',()=>setState(()=>_step=2)),const Spacer(),Align(alignment:Alignment.centerRight,child:_levelCard(s.width*.57,true)),const SizedBox(height:7),Align(alignment:Alignment.centerRight,child:SizedBox(width:s.width*.42,child:_continueLevelButton(height:46)))]);
  Widget _levelCard(double width,bool compact)=>Container(width:width,padding:EdgeInsets.all(compact?12:14),decoration:_cardDecoration(),child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisSize:MainAxisSize.min,children:[Text('DEIN STARTPUNKT',style:TextStyle(color:const Color(0xFFFF5BAE),fontSize:compact?11:12.5,fontWeight:FontWeight.w900)),SizedBox(height:compact?3:5),Text('Wo stehst du gerade?',style:TextStyle(color:Colors.white,fontSize:compact?19:22,fontWeight:FontWeight.w900)),SizedBox(height:compact?3:5),Text('${widget.guideName} passt deine ersten Schritte an dein aktuelles Niveau an.',style:TextStyle(color:const Color(0xFFF2EDF2),fontSize:compact?10.5:11.5,height:1.2)),SizedBox(height:compact?7:9),_twoByTwo([
    _levelOption('new',Icons.auto_awesome_rounded,'Ich starte ganz neu','Keine oder fast keine Vorkenntnisse',compact),
    _levelOption('words',Icons.chat_bubble_outline_rounded,'Ich kenne ein paar Wörter','Begrüßungen und einzelne Begriffe',compact),
    ...
    _levelOption('restart',Icons.refresh_rounded,'Ich möchte wieder einsteigen','Früher gelernt – jetzt auffrischen',compact),
  ])]));
  Widget _levelOption(String value,IconData icon,String title,String subtitle,bool compact){final selected=_selectedLevel==value;return _choiceTile(icon,title,subtitle,selected,()=>setState(()=>_selectedLevel=value),compact);}

  Widget _stepFourPortrait(Size s)=>Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[_header('4 / 8',()=>setState(()=>_step=3)),const Spacer(),Align(alignment:Alignment.centerLeft,child:_goalsCard(s.width*.76,false)),const SizedBox(height:12),_continueGoalsButton(height:58)]);
  Widget _stepFourLandscape(Size s)=>Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[_header('4 / 8',()=>setState(()=>_step=3)),const Spacer(),Align(alignment:Alignment.centerLeft,child:_goalsCard(s.width*.60,true)),const SizedBox(height:7),Align(alignment:Alignment.centerLeft,child:SizedBox(width:s.width*.44,child:_continueGoalsButton(height:46)))]);
  Widget _goalsCard(double width,bool compact)=>Container(width:width,padding:EdgeInsets.all(compact?12:14),decoration:_cardDecoration(),child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisSize:MainAxisSize.min,children:[
    Text('DEINE REISE',style:TextStyle(color:const Color(0xFFFF5BAE),fontSize:compact?11:12.5,fontWeight:FontWeight.w900)),
    SizedBox(height:compact?3:5),
    Text('Was möchtest du mit der Sprache erleben?',style:TextStyle(color:Colors.white,fontSize:compact?19:22,fontWeight:FontWeight.w900)),
    SizedBox(height:compact?3:5),
    Text('Wähle alles aus, was zu dir passt. ${widget.guideName} richtet deine Reise danach aus.',style:TextStyle(color:const Color(0xFFF2EDF2),fontSize:compact?10.5:11.5,height:1.2)),
    SizedBox(height:compact?7:9),
    _twoByTwo([
      _goalOption('travel',Icons.flight_takeoff_rounded,'Auf Reisen','Im Urlaub sicherer sprechen',compact),
      _goalOption('everyday',Icons.forum_outlined,'Im Alltag','Gespräche verstehen und führen',compact),
      _goalOption('culture',Icons.favorite_outline_rounded,'Menschen & Kultur','Sprache und Kultur wirklich erleben',compact),
      _goalOption('myself',Icons.explore_outlined,'Für mich selbst','Einfach eine neue Sprache lernen',compact),
    ]),
  ]));
  Widget _goalOption(String value,IconData icon,String title,String subtitle,bool compact){final selected=_selectedGoals.contains(value);return _choiceTile(icon,title,subtitle,selected,(){setState((){selected?_selectedGoals.remove(value):_selectedGoals.add(value);});},compact);}

  Widget _twoByTwo(List<Widget> items)=>Column(children:[Row(crossAxisAlignment:CrossAxisAlignment.start,children:[Expanded(child:items[0]),const SizedBox(width:7),Expanded(child:items[1])]),const SizedBox(height:7),Row(crossAxisAlignment:CrossAxisAlignment.start,children:[Expanded(child:items[2]),const SizedBox(width:7),Expanded(child:items[3])])]);
  Widget _choiceTile(IconData icon,String title,String subtitle,bool selected,VoidCallback tap,bool compact)=>Material(color:Colors.transparent,child:InkWell(onTap:tap,borderRadius:BorderRadius.circular(14),child:AnimatedContainer(duration:const Duration(milliseconds:180),constraints:BoxConstraints(minHeight:compact?64:78),padding:EdgeInsets.symmetric(horizontal:compact?7:9,vertical:compact?6:8),decoration:BoxDecoration(color:selected?const Color(0x3DFF2E9A):const Color(0xC91E1C22),borderRadius:BorderRadius.circular(14),border:Border.all(color:selected?const Color(0xFFFF2E9A):const Color(0xFF3C3740),width:selected?1.7:1)),child:Row(children:[Icon(icon,color:const Color(0xFFFF2E9A),size:compact?18:20),const SizedBox(width:7),Expanded(child:Column(mainAxisSize:MainAxisSize.min,crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title,maxLines:2,style:TextStyle(color:Colors.white,fontSize:compact?10.5:11.5,height:1.1,fontWeight:FontWeight.w800)),const SizedBox(height:2),Text(subtitle,maxLines:2,overflow:TextOverflow.ellipsis,style:TextStyle(color:const Color(0xFFD7D0D7),fontSize:compact?8.5:9.5,height:1.1))])),if(selected) const Icon(Icons.check_circle_rounded,color:Color(0xFFFF2E9A),size:17)]))));
  ...
  Widget _continueLevelButton({double height=62})=>_pinkButton('Weiter',_selectedLevel==null?null:()=>setState(()=>_step=4),height:height);
  Widget _continueGoalsButton({double height=62})=>_pinkButton('Weiter',_selectedGoals.isEmpty?null:(){ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Super, ${_nameController.text.trim()}! Deine Ziele sind gespeichert. Schritt 5 folgt als Nächstes.')));},height:height);
